...
def get_transforms(dataset, dataset_resolution=None, mode = None):  #mode == train, val, test
    if mode == 'train':
        if dataset_resolution == None:
            if dataset == 'STL10':
...
def get_train_valid_loader(data_dir,
    ...
    if shuffle:
        ...
        np.random.shuffle(indices_for_valid)
        np.random.shuffle(train_idx)

    train_idx, valid_idx = lb_idx, indices_for_valid[:split]
